...
import { taskStatusFilter, TASK_STATUS, DATA_SOURCE_ENUM, FLINK_VERSION_TYPE } from '@/constant';
...
class StreamTask extends React.Component<any, any> {
    ...
    loadCount() {
        ...
        const { status, taskType, strategyName, componentVersion }: Partial<IStreamTaskProps> = filter || {}
        Api.taskStatistics({
            taskName,
            statusList: status,
            type: taskType,
    ...
    updateTaskStatus = (task: IStreamTaskProps, mode?: string, isForce?: number) => {
        ...
        switch (status) {
            ...
            case TASK_STATUS.AUTO_CANCEL: {
                if (mode !== 'normal' && (status === TASK_STATUS.STOPED || status === TASK_STATUS.RUN_FAILED || status === TASK_STATUS.FINISHED || status === TASK_STATUS.AUTO_CANCEL)) { // 续跑
    ...
    getDealButton(record: IStreamTaskProps | undefined, isPane?: boolean) {
        ...
        switch (record.status) {
            ...
            case TASK_STATUS.FINISHED:
                goOn = notGoOn ? '' : '续跑'
                recover = <a>重跑</a>
                popTxt = <div style={{ width: 200 }}>重跑，则任务将丢弃停止前的状态，重新运行，若存在启停策略，将恢复自动启停</div>
                break;
            case TASK_STATUS.STOPED:
            case TASK_STATUS.KILLED:
                goOn = notGoOn ? '' : '续跑'
        ...
        if (isPane) {
            return (
                <span className="c-operation__btn">
                    <Button type="link" onClick={() => goToTaskDev({ id: record?.id })}>修改</Button>
                    {goOn ? <>
                        <span className="ant-divider" />
                        <Button type="link" onClick={() => { this.debounceUpdateTaskStatus(record) }}>{goOn}</Button>
                    </> : null}
                    {normal ? (normal == '停止' ? <>
                        <span className="ant-divider" />
                        <Button
                        ...
                        <Button
                            type="link"
                            onClick={() => { this.debounceUpdateTaskStatus(record, 'normal') }}
                        >
                            {normal}
                        </Button>
                    </>) : null}
                    {recover ? this.isFlinkSqlGuideMode(record)
                        ? <span>
                            <span className="ant-divider" />
                            <Button type="link" onClick={() => { this.setState({ taskId: record.id, reRunModalVisible: true }) }}>
                                {recover}
                            </Button>
                        </span>
                        : <Popconfirm
                            okText="确定"
                            cancelText="取消"
                            onConfirm={() => { this.debounceRecoverTask(record) }}
                            title={popTxt}
                        >
                            <span className="ant-divider" />
            ...
            arr.push(<a key="change" onClick={() => { goToTaskDev({ id: record?.id }) }}>修改</a>)
            goOn && arr.push(<a key="goon" onClick={() => { this.debounceUpdateTaskStatus(record) }}>{goOn}</a>);
            if (normal) {
                if (normal == '停止') {
                    arr.push(
                        <a key="normal" onClick={openModal}>{normal}</a>
                    )
                } else {
                    arr.push(<a key="normal" onClick={() => { this.debounceUpdateTaskStatus(record, 'normal') }}>{normal}</a>)
                }
            }
            recover && arr.push(this.isFlinkSqlGuideMode(record)
                ? <span>
    ...
    renderFooter = () => {
        ...
        return ([
            <Button
                key="submit"
                type="primary"
                onClick={this.handleReTaskRunning}
            >
                提交/重跑
            </Button>,
            <Button
    ...
    render() {
        const {
            tasks, selectTask, batchReRunVisible,
            reRunModalVisible, taskId, __modalKey,
            handTiedModelVisible, polling, goOnTask, visibleSlidePane
        } = this.state
            ...
            <div style={{ padding: 20, position: 'relative' }}>
                ...
                >
                    ...
                    <Radio.Group defaultValue={0}>
                        <Radio className="o-modal__radio" value={0}>使用上次任务参数重跑</Radio>
                        <Radio className="o-modal__radio--padding" value={1} disabled>指定Offset Time位置重跑</Radio>
                ...
                />
                {handTiedModelVisible && <HandTiedModal
